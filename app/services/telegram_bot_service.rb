...
class TelegramBotService
  ...
  def self.start_command(bot, message)
    ...
    bot.api.send_message(chat_id: telegram_chat_id, text: "Добро пожаловать! Введите номер телефона, который вы использовали при регистрации на сайте.")
  end
  ...
  def self.handle_phone_number(bot, message)
    ...
    begin
      ...
      if response.is_a?(Net::HTTPSuccess)
        ...
        bot.api.send_message(chat_id: telegram_chat_id, text: "Ошибка: не удалось связать Telegram с учетной записью. Проверьте введенный номер телефона.")
      end
    rescue StandardError => e
